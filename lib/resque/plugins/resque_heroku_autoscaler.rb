    ...
    module HerokuAutoscaler
      ...
      def set_workers(number_of_workers)
        if number_of_workers != current_workers
          heroku_api.post_ps_scale(config.heroku_app, 'resque', number_of_workers)
        end
      end

      def current_workers
        heroku_api.get_ps(config.heroku_app).body.count {|p| p['process'].match(/resque\.\d+/) }
      end
      ...
      def time_to_scale?
        (Time.now - Time.parse(Resque.redis.get('last_scaled') || "2012-01-01")) >=  config.wait_time
      end
